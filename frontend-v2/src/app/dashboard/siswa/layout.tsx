...
const NAV_ITEMS = [
    ...
    { href: "/dashboard/siswa/quizzes", label: "Quiz", icon: "quiz" },
    { href: "/dashboard/siswa/adaptif", label: "Belajar Adaptif", icon: "school" },
    { href: "/dashboard/siswa/rewards", label: "Hadiah", icon: "redeem" },
    { href: "/dashboard/siswa/history", label: "Riwayat", icon: "history" },
    { href: "/dashboard/siswa/profile", label: "Profil", icon: "person" },
];
...
export default function StudentDashboardLayout({
    ...
    const isFullscreenPage = pathname?.includes('/exam') || pathname?.includes('/review') || pathname?.includes('/adaptif/');

    if (isFullscreenPage) {
        // For fullscreen pages, render without padding and navigation
        return (
            <div className="min-h-screen bg-[#fdfbf7] font-[var(--font-lexend)] text-[#0f172a] overflow-x-hidden relative">
                {children}
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-[#fafafa] font-[var(--font-lexend)] text-[#171717] overflow-x-hidden relative">
            {/* Sidebar desktop */}
            <aside className="fixed inset-y-0 left-0 z-40 hidden w-60 flex-col border-r border-[#e5e5e5] bg-white md:flex">
                <div className="flex h-16 items-center gap-3 border-b border-[#e5e5e5] px-5">
                    <span className="material-symbols-outlined text-2xl text-[#c9a227]">school</span>
                    <div>
                        <p className="text-sm font-bold leading-tight">TemanUjian</p>
                        <p className="text-xs text-[#a3a3a3]">Panel Siswa</p>
                    </div>
                </div>
                ...
                <div className="border-t border-[#e5e5e5] p-3">
                    <div className="flex items-center gap-3 rounded-md px-3 py-2">
                        ...
                        <div className="min-w-0 flex-1">
                            <p className="truncate text-sm font-semibold">{user?.username || "Siswa"}</p>
                            <p className="text-xs text-[#a3a3a3]">{user?.kelas || "Siswa"}</p>
                        </div>
                    </div>
                    <button
                        onClick={() => logout()}
            ...
            <header className={`fixed inset-x-0 top-0 z-30 h-16 border-b border-[#e5e5e5] bg-white/95 backdrop-blur-sm transition-all md:left-60 ${scrolled ? "shadow-sm" : ""}`}>
                <div className="flex h-full items-center justify-between px-4 md:px-6">
                    <p className="text-sm font-bold md:text-base">Dashboard Siswa</p>
